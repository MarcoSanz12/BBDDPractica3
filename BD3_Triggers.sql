...
BEGIN
    ...
    ELSEIF (new.evaluacion = 3 AND @anyo = 2 AND (new.tipo = 'fct' OR new.tipo = 'proyecto')) THEN
        ...
        SET @fct:=(SELECT count(*) FROM nota WHERE n.idAlumno = new.idAlumno AND n.tipo = 'fct');
        
        SET @proyecto:=(SELECT count(*) FROM nota WHERE n.idAlumno = new.idAlumno AND n.tipo = 'proyecto');
